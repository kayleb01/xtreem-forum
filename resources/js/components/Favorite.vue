<template>
    <a href="#" @click.prevent="toggle" class="inline-block text-center px-3">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="16" viewBox="0 0 20 19" class="fill-current d-inline" :class="active ? 'text-green' : 'text-grey-light'">
        ...
        <span v-if="count">
            <span v-text="count"  :class="active ? 'text-danger' : 'text-grey'"></span>
        </span>
    </a>
</template>
...
<script>
    export default {
        ...
        computed: {
            endpoint() {
                return '/comment/like/' + this.comment.id + '';
            }
        },
